...
public class AssistantService {
    ...
    private static final String SYSTEM_PROMPT =
        ...
        "%s\n\n" +
        "回答规则（必须严格遵守）：\n" +
        "1. 开头必须写明：根据您查询的\"<用户问题的核心关键词>\"，在定额库中匹配到以下相关定额：\n" +
        "2. 根据用户问题，从上述定额数据中做语义匹配，找出所有相关的定额，不得遗漏任何版本\n" +
        "3. 匹配时重点关注：定额名称的语义相似度、项目特征值的技术参数一致性、单位是否匹配\n" +
        "4. 【强制】所有匹配到的定额必须全部列出，按编号顺序呈现。每条定额严格按以下格式输出：\n" +
        "   序号. 版本名 | 定额名称 | 单位：<单位> | 单价：<单价>元 | 备注 <备注信息>【版本名】\n" +
        "   示例：\n" +
        "   根据您查询的\"3*2.5\"规格，在定额库中匹配到以下相关定额：\n\n" +
        "   1. 2025改造定额 | 电力电缆（NH-YJV22-3×2.5） | 单位：米 | 单价：2.33元 | 备注 综合考虑敷设方式【2025改造定额】\n" +
        "   2. 2025新建定额 | 电力电缆 WDZN-YJY-3*2.5 | 单位：米 | 单价：2.42元 | 备注 综合考虑敷设方式【2025新建定额】\n" +
        "   3. 2025新建定额 | 电力电缆（NH-YJV22-3×2.5） | 单位：米 | 单价：2.33元 | 备注 综合考虑敷设方式【2025新建定额】\n\n" +
        "   以上均为3芯2.5平方毫米规格的电力电缆，单价略有差异，您可根据具体项目类型（改造或新建）及电缆型号进行选择。\n" +
        "5. 结尾必须附加一句总结，对匹配到的定额进行简要比较分析，帮助用户选择\n" +
        "6. 如果确实没有相关定额，回复\"暂未在定额库中找到与该问题相关的定额\"\n" +
        "7. 绝对不要使用定额数据以外的知识进行猜测或补充\n" +
    ...
    public Map<String, Object> chat(String question, Long userId) {
        ...
        // 6. 调用 DeepSeek API（追加指令：格式、全版本覆盖）
        String enhancedQuestion = "请回答以下问题。核心要求：\n" +
                "1. 开头写：根据您查询的\"<关键词>\"，在定额库中匹配到以下相关定额：\n" +
                "2. 每条定额格式：序号. 版本名 | 定额名称 | 单位：XX | 单价：XX元 | 备注 XX【版本名】\n" +
                "3. 结尾加一句总结分析，帮助用户比较选择\n" +
                "4. 在所有可用版本中搜索，不要遗漏任何版本\n" +
                "\n问题：" + question;
