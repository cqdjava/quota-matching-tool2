...
public class AssistantService {
    ...
    private static final String SYSTEM_PROMPT =
        ...
        "以下是系统中所有企业定额数据（格式：编码 | 名称 | 特征值 | 单位 | 单价）：\n\n" +
        "%s\n\n" +
        "回答规则：\n" +
        "1. 根据用户问题，从上述定额中做语义匹配，找到最相关的定额\n" +
        "2. 匹配时重点关注：定额名称的语义相似度、项目特征值的技术参数一致性、单位是否匹配\n" +
        "3. 回答中必须引用具体定额，格式：\"根据【编码】名称，单价为...\"\n" +
        "4. 如果有多条相近定额，列出供用户选择，说明每条定额与问题的关联点\n" +
    ...
    public Map<String, Object> chat(String question, Long userId) {
        ...
        if (allQuotas.size() > MAX_QUOTAS_IN_PROMPT) {
            quotasForPrompt = roughFilter(question, allQuotas);
        } else {
            quotasForPrompt = allQuotas;
        }
        ...
        // 6. 调用 DeepSeek API
        String llmResponse = callDeepSeekApi(systemPrompt, question);
    ...
    private String buildQuotaIndex(List<EnterpriseQuota> quotas, Map<Long, String> versionNameMap) {
        ...
        // 按版本分组
        Map<String, List<EnterpriseQuota>> byVersion = new LinkedHashMap<>();
    ...
    private List<EnterpriseQuota> roughFilter(String question, List<EnterpriseQuota> allQuotas) {
        ...
        for (EnterpriseQuota q : allQuotas) {
            String text = (q.getQuotaName() != null ? q.getQuotaName() : "") + " " +
